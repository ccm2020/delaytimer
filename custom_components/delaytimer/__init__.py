...
SERVICE_REMAIN = "remain"
SERVICE_EQUERY = "set_equery"
...
async def async_setup(hass, config):
    ...
    component.async_register_entity_service(SERVICE_EQUERY,{ATTR_EQUERY:dict},"async_equery")
    ...
    component.async_register_entity_service(SERVICE_REMAIN, {}, "async_remain")
...
class Timer(RestoreEntity):
    ...
    def __init__(self, hass, object_id, name, icon, duration):
        ...
        self._listener = None
        self._equery = {}
        """async_track_time_interval(self._hass, self.async_remain, timedelta(seconds=0.5))"""
    ...
    async def async_equery(self, equery):
        self._equery = equery
    ...
    async def async_finished(self, time):
        ...
        for entity_id,action in self._equery.items():
            entity_domain = entity_id.split('.',1)[0]
            entity_state = self._hass.states.get(entity_id).state
            if action == 'turn_on':
                action_state = 'on'
                ...
                action_state = action
            if entity_state != action_state:
                self._hass.async_create_task(self._hass.services.async_call(entity_domain,action,{"entity_id": entity_id}))
